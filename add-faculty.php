<?php
    ...
    if(isset($_POST["send"])){
       ...
       $sql="SELECT * FROM faculty WHERE fact_name='$faculty_name'";
       $query=mysqli_query($con, $sql);
       $count=mysqli_num_rows($query);
       if($count < 1){
            $sql1="INSERT INTO faculty (fact_name, fact_status) VALUES('$faculty_name', '1')";
    ...
    if(isset($_GET["delete"])){
        $id=$_GET["delete"];
        //echo $delete; 
        $sql="DELETE FROM faculty WHERE fact_id='$id'";
        $query=mysqli_query($con, $sql);
        if($query){
            echo "<script> 
                alert('Record deleted successfully');
                document.location.href='add-faculty.php';
                </script>";
        }else{
            ...
            <div class="col-5 border mb-3 shadow" style="margin:0 auto">
                ...
                <form action="" method="post" style="" class="pt-4 pb-4">
                    <div class="form-group row">
                        ...
                        <div class="col-9">
                            <input type="text" name="faculty_name" required class="form-control-sm" placeholder="Enter faculty name">  
                        </div>
                    </div>
                    <div class="form-group row">
        ...
        <div class="row">
            <div class="col-12">
                <table class="table table-border table-striped">
                    <thead>
                        <tr>
                            <th>#</th>  <th>Faculty name</th>  <th colspan="1">Action</th> 
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                        ...
                        ?>
                        <tr>
                            <td><?php echo $i; ?></td> <td><?php echo $result2["fact_name"] ;?></td> 
                            <td>
                                <a href="add-faculty.php?delete=<?php echo $result2["fact_id"]?>"
